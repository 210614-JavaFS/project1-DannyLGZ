const URL = 'http://localhost:8080/ers/employee/';
...
async function checkSessions(){
    ...
    if (response.status == 404){
        ...
        document.getElementById("userRole").innerText = user.roleId.role;
        if (user.roleId.role == "Finance Manager"){
            document.getElementById("allReimButton").hidden=false;
...
function populatePendingReimList(data){
    ...
    for(let pending of data){
        ...
        // let tdReimResolved = document.createElement("td");
        // tdReimResolved.innerText = pending["reimResolved"];
        // row.appendChild(tdReimResolved);

        // let tdReimDescription = document.createElement("td");
        // tdReimDescription.innerText = pending["reimDescription"];
        // row.appendChild(tdReimDescription);
        ...
        // let tdReimResolver = document.createElement("td");
        // tdReimResolver.innerText = pending["reimResolverId"].firstName + " "+ pending["reimResolverId"].lastName;
        // row.appendChild(tdReimResolver);

        // let tdReimStatus = document.createElement("td");
        // tdReimStatus.innerText = pending["reimStatusId"].status;
        // row.appendChild(tdReimStatus);
...
async function getMyReimList(){
    ...
    if(response.status === 200){
        ...
        document.getElementById("allreimtable").hidden = true; //fix here
        document.getElementById("pendingtable").hidden = true;      
